...
dir.create("./FBA_GAMMS/Total_sample/output")

# MODELS WITH COVARS (FULL MODEL)

dir.create("./FBA_GAMMS/Total_sample/output/fullmodel")
setwd("./FBA_GAMMS/Total_sample/output/fullmodel")
...
models<-lapply(X=as.character(as.list(dvstotal)),
               ...
               FUN=function(dv_name, df) {
                ...
                # ordered factoring of med
                 ...
                 gam1a <- gam(value  ~ 1 + SES_c + OFsex + OFmed + meanFWD_c + etiv.long_c + OFscanner + s(SID,bs='re'), data=adf, method = "ML") # add 'family' option
                 # Model with smooth Age predictor
                 gam2a <- gam(value  ~ s(age_c,bs="cr",k=4) + SES_c + OFsex + OFmed + meanFWD_c + etiv.long_c + OFscanner + s(SID,bs='re'), data=adf, method = "ML") # add 'family' option
                 #Model with smooth age predictor and group
                 gam3a <- gam(value  ~ s(age_c,bs="cr",k=4) + OFgroup + SES_c + OFsex + OFmed + meanFWD_c + etiv.long_c + OFscanner + s(SID,bs='re'), data=adf, method = "ML") # add 'family' option
                 # Model with smooth age and group and interaction with age and group
                 gam4a <- gam(value  ~ s(age_c,bs="cr",k=4) + OFgroup + s(age_c, by=OFgroup, bs='cr',k=4) + SES_c + OFsex + OFmed + meanFWD_c + etiv.long_c + OFscanner + s(SID,bs='re'), data=adf, method = "ML") # add 'family' option
                 ...
                 if (pvalue1 < 0.05) {
                   if (pvalue2 < 0.05) {
                     if (pvalue3 < 0.05) {
                       ...
                       plotdf3 <- data.frame(age_c=rep(age_c,2),OFgroup=c(rep("ADHD",length(age_c)),rep("CONTROL",length(age_c))),SID=c(rep("86",length(age_c)),rep("307",length(age_c))), SES_c = 0, OFsex = "female", meanFWD_c = 0, etiv.long_c = 0, OFmed = "no", OFscanner = "Tim_Trio")
                     ...
                     plotdf2 <- data.frame(age_c=rep(age_c,2),OFgroup=c(rep("ADHD",length(age_c)),rep("CONTROL",length(age_c))),SID=c(rep("86",length(age_c)),rep("307",length(age_c))), SES_c = 0, OFsex = "female", meanFWD_c = 0, etiv.long_c = 0, Ofmed = "no", OFscanner = "Tim_Trio")
                    ...
                    #predictor for model gam2a (Age)
                                     plotdf <- data.frame(age_c=rep(age_c,2),OFgroup=c(rep("ADHD",length(age_c)),rep("CONTROL",length(age_c))),SID=c(rep("86",length(age_c)),rep("307",length(age_c))), SES_c = 0, OFsex = "female", meanFWD_c = 0, etiv.long_c = 0, OFmed = "no", OFscanner = "Tim_Trio")
...
FBA_models_total <- rbindlist(models,fill=TRUE)
View(FBA_models_total)
write.csv(FBA_models_total,file= "total_FBA_models.csv",row.names=F)


modelsFDR <- FBA_models_total[,c(1,6,7,10,14,18,19)]
modelsFDR$p.fdr <- p.adjust(modelsFDR$`Pr(>|t|)`,method="fdr")
modelsFDR$pvalue.fdr <- p.adjust(modelsFDR$`p.value`,method="fdr")
modelsFDR$`p-value.fdr` <- p.adjust(modelsFDR$`p-value`,method="fdr")
write_csv(modelsFDR,path = "total_FBA_models_FDR.csv")
